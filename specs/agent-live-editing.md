...
Agent MCP edits (`create_mesh`, `set_transform`, the `sculpt_*` pipeline, …) render **live in the app's 3D viewport**, in the current window, instead of landing in a separate headless process. The CLI is **visible by default**: on an interactive session the first stage-mutating tool call auto-launches USDZ Studio on the served document so the user watches the build live, without having to open the app first or call `open_in_app`. `--headless` opts out (windowless; still relays to an editor the user opens themselves), and `--no-relay` remains the deterministic in-process path for CI/scripting.
...
- **Visible by default — auto-launch on first mutation.** Routing only ever *observed* editor liveness; it never *started* the app, so a session begun with the app closed ran entirely in-process and invisible until the agent happened to call `open_in_app` (which only opens a static snapshot). Now `AdaptiveTransport.run` takes an `autoLaunch` flag (on unless `--headless`) and a `launch` thunk: the first stdin line that `AutoLaunch.shouldLaunch` classifies as a stage mutation — while no editor is live and no launch has been attempted — fires `AutoLaunch.launchApp` (`/usr/bin/open -a "USDZ Studio" <file>`, so the served document becomes the app's hosted stage via `.onOpenURL` → `bindDocument`) and then waits up to ~10 s (`RelayCodec.awaitEndpoint`) for the endpoint before routing, so that very call relays into the live document instead of racing it. The launch is attempted **at most once** per session; if the app never comes up the line falls back to in-process serving (degrade to headless, never hang). The mutation-vs-inspection decision is a pure allowlist (`AutoLaunch.nonMutatingTools`): read-only inspection at session start (`capabilities`, `query_scene`, `validate`, sculpt/rig probes, `open_in_app`, …) does not launch; everything else — including any newly-added editing tool — does (fail toward visibility). `AutoLaunch.shouldLaunch`/`isMutatingToolCall` and `RelayCodec.toolCallName` are pure and unit-tested; the `open` shell is coverage-disabled.
...
- **Discovery** is unchanged in shape (`…/USDZStudio/mcp/endpoint.json`) but the
...
- `CLI/Sources/AdaptiveTransport.swift` — `route(editorLive:)` (pure per-request decision, unit-tested) + `run(endpointURL:autoLaunch:launch:makeInProcessServer:)` (the stdin loop that re-decides per line and fires the one-shot auto-launch, coverage-disabled) + `InProcessHost` (lazily-built headless server + teardown hook).
- `CLI/Sources/AutoLaunch.swift` — `shouldLaunch`/`isMutatingToolCall`/`nonMutatingTools` (pure visible-by-default decision, unit-tested) + `launchApp` (the `/usr/bin/open -a "USDZ Studio"` shell, coverage-disabled) + `appName`.
...
- `CLI/Sources/McpCommand.swift` — `run` (routes: `--no-relay` → eager headless; else `AdaptiveTransport.run` with `autoLaunch: !headless`) + `resolve` (`--headless`/`--no-relay` flag parsing) + `makeInProcessHost` (builds the resident-bridge server + reference hand-off, lazily).
...
- `App/Sources/USDZStudioApp.swift` — binds the host on launch and on document change; passes `mcp.referenceModel` into the shell.
...
- `Packages/RenderKit/.../NativeRenderer.swift` — the concrete `RenderExecuting` backends (`NativeSceneKitRenderer`, `UsdrecordRenderer`, `NativeRendererSelection`, and the pure `RenderStageParse`). Both the CLI (`McpCommand`) and the App (`MCPActivityListener`) inject one, so a hosted server can satisfy `render_views` and the sculpt review loop without `usdrecord` (#109). Previously the renderer lived in `CLI/` and the app built a Configuration with none, so in-app renders failed.

### Unified render path — the agent sees the user's viewport

The app-hosted MCP server renders `render_views` through the **same RealityKit pipeline the user sees on screen**, not the separate SceneKit backend. This is the sanctioned "extend the viewport with a headless snapshot" path (CLAUDE.md: ONE renderer); the SceneKit backend is retained only as the headless fallback.

- `Packages/ViewportKit/.../ViewportSnapshotRenderer.swift` — `ViewportSnapshotRenderer` (public, `@MainActor`) renders a USD file offscreen through the viewport's own `ViewportCoordinator` (`Entity.load` + the QuickLook `.arPreview` key/fill rig) and returns PNG bytes. It hosts an `InteractiveARView` in a never-shown borderless `NSWindow` so RealityKit has a real drawable, then uses `ARView.snapshot`. `HeadlessCameraMath` (same file, pure, unit-tested) maps focal length → vertical FOV and the render tool's row-major camera transform → a simd camera-to-world matrix. New `ViewportCoordinator.loadEntitiesForSnapshot(url:)` / `applyCameraMatrix(_:fovDegrees:)` hooks feed it. The GPU path is `coverage:disable` (needs a GPU + window server; verified on a real machine via the `verify` skill / golden-image harness, never in CI).
- `App/Sources/ViewportKitRenderer.swift` — `ViewportKitRenderer: RenderExecuting` adapts the file-based contract onto `ViewportSnapshotRenderer`, parsing the authored camera via `RenderKit.RenderStageParse`. It lives in the App target because only the composition root may import both `ViewportKit` and `AgentMCP` (the module layering forbids either kit importing the other).
- `RenderKit.MCPRenderBackendSelection` — pure, unit-tested policy. `preferViewportRenderer` is the **hardcoded ON flag**; `backend(canUseViewport:)` returns `.viewport` for the app host (a live GPU/window context) and `.native` for the headless CLI. `MCPActivityListener.makeHost` branches on it, injecting `ViewportKitRenderer()` app-hosted and falling back to `NativeRendererSelection.make(...)` otherwise. Flip `preferViewportRenderer` to `false` to force every host back onto SceneKit.
...
- Unit: `AdaptiveTransportTests` (per-request `route(editorLive:)` decision, recomputed each call), `AutoLaunchTests` (visible-by-default `shouldLaunch`/`isMutatingToolCall` allowlist + gates), `SharedSessionTests` (shared-stage init), `RelayCodecTests` (frame encode/decode/drain + `toolCallName`), `UnixSocketTests` (path-fits + socket/endpoint path derivation), `SocketEventSinkTests` (socket-path endpoint decode). Reference panel: `ReferenceImageToolTests` (set/clear/errors, callback, `usd://reference`, `reference.json` round-trip), `MCPActivityPathsTests` (hand-off path), `ReferenceImageModelTests` (panel state). Gates: `dependency-lint.sh`, `module-governance.sh`, coverage (AgentMCP 100%, CLI ≥95%).
- End-to-end (empty window — the common "launch and sculpt" flow): launch the app (fresh build) with **no document open**; **reconnect Claude Code's MCP**; run `create_mesh` — a document is created automatically and the mesh appears live in the viewport; subsequent edits and `⌘Z` work against it.
...
- End-to-end (visible by default): with the app **closed**, start `openusdz mcp` (no `--headless`); issue read-only calls (`capabilities`, `query_scene`) and confirm **no window opens**; then issue the first mutation (`create_mesh`) and confirm USDZ Studio **auto-launches** on the served document and the mesh appears live. With `--headless`, the same first mutation stays windowless.
