...
### Milestone 5 — Validation & scripting power tools (finishes Phase 4) — ✅ **Done**
All three power tools shipped; see the Phase 4 entries for detail.
- Python console REPL with injected `stage`/`selection`/`app` and single-undo script runs — ScriptingKit `ReplSession` core plus the EditorUI console: `ReplController` writes the live stage to a temp file, runs one interpreter process per submission, re-opens the result, and records any change as one undoable `ReplaceStageCommand` (EditingKit, expressed via existing `StageMutation` vocabulary so it journals to the WAL). `ConsolePanel` + Console action button/⇧⌘P wire it to the live `EditorDocument.applyConsoleEdit(after:label:)`.
- Completed the live diagnostics quick-fix set (metersPerUnit / defaultPrim / upAxis re-orientation; topology/normals/unbound/duplicate-name intentionally no-fix) and wired the export path through `ComplianceChecker` gating: `EditorDocument.exportCompliance(profile:)` drives an ExportPanel pass/blocked banner (disabling export when blocked) and a hard gate in `runExport` that refuses a blocking export.
- FBX support via checksum-verified FBX2glTF download flow — `FBXImporter` shells out (behind an injected runner seam) to the binary fetched by `scripts/fetch-fbx2gltf.sh` (SHA-256 verified), then reuses `GLTFImporter`; registered in `ImporterRegistry.standard` so CLI + conversion sheet pick up `.fbx` automatically.
- **Exit / harness:** `ReplaceStageCommandTests` + `ReplControllerTests` (single-undo contract, no Python); `FBXImporterTests` cover every path/error via a fake runner (ConversionKit stays 100%); EditingKit holds 100%.
...
- [x] ComplianceChecker (ARKit profile) integration, export gating — `ValidationProfile` (named catalog + `blockingSeverity`): `.arkit` (blocks on error), `.arkitStrict` (blocks on warning); `ComplianceChecker` runs a profile → `ComplianceResult` with `isExportAllowed`/`blockingDiagnostics`/`summary` for the export path. CLI `validate` now takes `--profile NAME` (arkit|arkit-strict) with `--strict` as shorthand and conflict-guarded, gating exit code on the profile's decision. Drawer still reads the engine directly; the app **export flow is now gated**: `EditorDocument.exportCompliance(profile:)` drives an ExportPanel pass/blocked banner and a hard refusal in `runExport` when the profile blocks (Milestone 5)
- [x] Python console (REPL, injected `stage`/`selection`/`app`, single-undo script runs) — ScriptingKit REPL core (`ReplSession` actor: multi-line buffering via `ReplInputClassifier`, `ReplHistory` recall, `ReplProgram` injected-namespace builder binding `stage`/`selection`/`app`, `ReplEntry` transcript; one submission = one interpreter run = one undoable unit) **plus the EditorUI console**: `ReplController` (`@Observable`, 100%-covered with fakes) syncs the live stage to a temp file, runs one process per submission, re-opens the result, and records any change through `EditorDocument.applyConsoleEdit` as a single undoable `ReplaceStageCommand` (EditingKit, built from existing `StageMutation`s so it journals to the WAL like every command). `ConsolePanel` + a Console action button and ⇧⌘P menu item surface it
...
- [x] FBX support via checksum-verified FBX2glTF download flow — `FBXImporter` (ConversionKit) shells out behind an injected `FBX2glTFRunning` seam to the binary fetched + SHA-256-verified by `scripts/fetch-fbx2gltf.sh`, feeds the produced glTF to `GLTFImporter`, and is registered in `ImporterRegistry.standard` so CLI convert + the conversion sheet accept `.fbx` with no per-format code. 100%-covered against a fake runner (real `Process` spawn excluded with a reviewed annotation)
