...
- [~] **`Decimate`** (shipped, #127): pure-Swift QEM (Garland–Heckbert) edge-collapse decimation in MeshKit — no pymeshlab/Python dependency. Targets a keep-ratio or absolute triangle budget with an optional QEM `maxError` cap; boundary loops and UV-seam vertices are pinned (frozen) by default so silhouettes and texture layout survive; every collapse is guarded by the manifold link condition + a normal-flip test. Surfaced in mesh edit mode (`d`, ⌘K "Decimate / Reduce Mesh") and `edit_mesh {op: decimate}` over MCP. Euler/manifold/winding invariant harness, bounded-volume-drift check, in the fuzz rotation, 100%-gated. Remaining: LOD-set authoring (USD variants) + file-size "fit to budget" batch CLI. Subdivision-surface **preview** + bake-to-mesh (Catmull-Clark) still open.
